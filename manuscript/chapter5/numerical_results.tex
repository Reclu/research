...
Nevertheless, the numerical integration of ODEs once the shear stress $\sigma_{12}$ vanishes is not possible owing to an undeterminancy of the loading function $\psi_1^f$ that has not been identified so far.
...
On the other hand, some loading paths resulting from the integration of slow waves ODEs are depicted in figure \ref{fig:slow_path_plane_strains}.
...
%Hence, three different initial values are studied separately, that is $\sigma_{22}=-1.3 \times 10^{8} \: Pa$, $\sigma_{22}=0$ and $\sigma_{22}=1.3 \times 10^8 \: Pa$. 
...
\begin{figure}[h!]
  \centering
  \subcaptionbox{Projections of loading paths in ($\sigma_{11},\sigma_{12}$) and ($\sigma_{22},\sigma_{12}$) planes \label{subfig:slowDP_stress1}}{\input{chapter5/pgfFigures/DPslowWaves2.tex}}
  \subcaptionbox{Loading path in deviatoric plane \label{subfig:slowDP_dev1}}{\input{chapter5/pgfFigures/DPslowWaves_deviator2.tex}}
  \caption{Loading paths through slow simple waves for different starting points on the initial yield surface.}
  \label{fig:slow_path_plane_strains}
\end{figure}
% \begin{figure}[h!]
%   \centering
%   \subcaptionbox{Projections of loading paths in ($\sigma_{11},\sigma_{12}$) and ($\sigma_{22},\sigma_{12}$) planes \label{subfig:slowDP_stress1}}{\input{chapter5/pgfFigures/DPslowWaves3.tex}}
%   \subcaptionbox{Loading path in deviatoric plane \label{subfig:slowDP_dev1}}{\input{chapter5/pgfFigures/DPslowWaves_deviator3.tex}}
%   \caption{Loading paths through slow simple waves for different starting points on the initial yield surface.}
%   \label{fig:slow_path_plane_strains}
% \end{figure}
...
Indeed, it can be seen in figure \ref{fig:pureShear}\subref{subfig:pureShear1} that the integral curve in $(s_3,\sigma_{22})$ plane break down in a first step in which both stresses vary, followed by one in which $s_3$ only is constant and equal to zero.
%Nevertheless, it can be seen in figure \ref{fig:pureShear}\subref{subfig:pureShear1} that the pure shear state in deviatoric plane is achieved before the stress $\sigma_{22}$ stops varying.
%Indeed, the starting points of every loading paths correspond to the intersection of curves with the $s_3$-axis in such a way that a first phase occurs during which $\sigma_{22}$ along with $s_3$ vary until the third eigenvalue of the stress deviator is zero.
%The second phase then consist in a variation of $\sigma_{12}$ whereas $s_3$ remains null.
%% evolution de la pression hydrostatique
The loading paths are, however, not restricted to the deviatoric plane as can be seen in figure \ref{fig:pureShear}\subref{subfig:pureShear2}, which shows the evolution of the hydrostatic pressure $\sigma = \tr(\tens{\sigma})$ with respect to the stress $\sigma_{22}$.
...
Finally
%Figure \ref{fig:pureShear}\subref{subfig:pureShear2} shows on the other hand the evolution of the hydrostatic pressure $\sigma = \tr(\tens{\sigma})$ with respect to the stress $\sigma_{22}$. 

%The curves in this figure do not admit no vertical nor horizontal asymptots so that once the hydrostatic pressure stops varying, the stress $\sigma_{22}$ is also constant.
\begin{figure}[h!]
  \centering
  {\phantomsubcaption \label{subfig:pureShear1}}
  {\phantomsubcaption \label{subfig:pureShear2}}
  \input{chapter5/pgfFigures/pureShearReached}
...
% \begin{figure}[h!]
%   \centering
%   \input{chapter5/pgfFigures/LodeAngle}
%   \caption{Evolution of Lode parameter along the loading path followed in slow simple waves depicted in figure \ref{fig:slow_path_plane_strains}.}
%   \label{fig:lode}
% \end{figure}
...
% \begin{figure}[h!]
%   \centering
%   \input{chapter5/pgfFigures/maxCriterion}
%   \caption{Evolution of Lode parameter along the loading path followed in slow simple waves depicted in figure \ref{fig:slow_path_plane_strains}.}
%   \label{fig:lode}
% \end{figure}

%% Il faut encore réfléchir à la manière d'identifier le comportement a sigma22=Cte en terme de tracés de courbes. Ca devrait modifier les conclusions...
% Thus, the integral curves are represented in figure \ref{fig:lode} in the $\sigma_{12},\sigma_{22}$ along with the $\Theta-\sigma_{12}$ curves.
% As can be seen, the asymptotic limit of Lode angle $\Theta=30$ if reached before these of stress $\sigma_{22}=C^{te}$ for paths $1$ and $4$.
% As a consequence, the longitudinal stress $\sigma_{22}$, as well as $\sigma_{11}$, still vary along the radial direction in the deviatoric plane until some limit value is achieved.
% Once this point is reached, only the shear component $\sigma_{12}$ varies (at least more significantly than the longitudinal components).
%Unfortunately, this singular stress state has not been identified so far.
%As a matter of fact, it would be crucial to determine this state beyond which the slow waves are no longer combined loads ones but pure shear waves.

%The same remarks hold for the two other initial conditions on $\sigma_{22}$, the loading paths of which can be seen in figure \ref{fig:slow_path_plane_strains2}.% and \ref{fig:slow_path_plane_strains3}.
% \begin{figure}[h!]
%   \centering
%   \subcaptionbox{Slow waves loading paths projections for $\sigma_{22}=0$ \label{subfig:slowDP2}}{
...
%   \newline
%   \subcaptionbox{Slow waves loading paths projections for $\sigma_{22}=1.3 \times 10^{8} \: Pa$ \label{subfig:slowDP3}}{
...
%   \caption{Loading paths through slow simple waves with initial condition $\sigma_{22}=0$ for different starting points on the initial yield surface.}

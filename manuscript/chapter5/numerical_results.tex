...
On the other hand, the tensile yield stress $\sigma^y=1 \times 10^{8} \: Pa$ and the hardening modulus $H=1\times10^8 \: Pa$ are set here arbitrarily.
...
\begin{equation}
  \label{eq:flow_rule}
  \tens{\dot{\eps}}^p = \frac{3}{2H}\frac{\tens{s}\otimes\tens{s}}{\norm{\tens{s}}^2} :\tens{\dot{\sigma}}
\end{equation}
...
%If on the other hand, the hardening parameter is raised to $H=1\times10^{10} \: Pa$, the loading paths slightly differ.
...
The curves resulting from the use of the hardening parameter $H=1\times10^{10} \: Pa$ exhibit slight differences.
...
\begin{figure}[h!]
  ...
  \caption{Fast simple wave solutions of the plane strain problems $H=1\times10^{10} \: Pa$.}
  \label{fig:fast_H}
\end{figure}
...
\begin{figure}[h!]
  ...
  \caption{Slow simple wave solutions of the previous problems with $H=1\times10^{10} \: Pa$.}
  \label{fig:slow_H}
\end{figure}

...
%of the thin-walled cylinder studied by Clifton \cite{Clifton}.
...
\end{equation}
where the loading functions $\psi^{s,f}$ depend on the component of the acoustic that corresponds to the tangent modulus \eqref{eq:ch5_TW_tangent}.
Equations \ref{eq:tw_paths} as well as these of Clifton \cite{Clifton} have been numerically integrated, starting from several arbitrary points lying on the initial yield surface.
Since the loading functions are odd functions of $\sigma$ and $\tau$ \cite{Clifton}, the loading paths exhibit symmetries with respect to $\tau$ and $\sigma$ axes.
...
Nonetheless, this property holds in $(\sigma,\tau)$ plane but not in the deviatoric plane as can be seen in figure \ref{fig:tw_slow}\subref{subfig:tw_slow_dev} since the ODEs are not written in terms of $s_1,s_2,s_3$.
...
\end{equation}
In addition, the plastic flow rule \eqref{eq:plastic_strain_rate} reads:
\begin{equation}
  \label{eq:flow_rule}
...
\end{equation}
Thus, the system of ODE consist of the equations of table \ref{tab:simpleWavesEquations}:
\begin{align}
...
In contrast, it can be seen in figure \ref{fig:slow_path_plane_strains1}\subref{subfig:slowDP_dev1} that the paths first follow the initial yield surface and then a direction of pure shear in the deviatoric plane, which differs from plane stress solutions.
...
Namely, the loading paths are restricted to the $(\sigma_1,\sigma_2)$ under plane stress while they can take values in the whole space ($\sigma_1,\sigma_2,\sigma_3$) under plane strain.
...
Conversely, $\sigma_{22}$ decreases along the loading path if $\sigma_{11}$ is initially lower than the value corresponding to the maximum shear stress on the initial yield surface.
...
The curves resulting from the use of the tangent modulus $C=1\times10^{10} \: Pa$ exhibit slight differences.
...
As a result, the stress paths followed inside slow simple waves depicted in figure \ref{fig:slow_H} lead to stress state lying further outside of the initial elastic convex.
\begin{figure}[h!]
  \centering

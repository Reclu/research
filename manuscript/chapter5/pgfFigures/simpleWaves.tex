\begin{tikzpicture}[scale=1.] 
  ...
  % Slope = 0.75
  \draw (0,0) -- (4,3.) node [right] {$c_s^0$};
  % Slope = 4./3.
  \draw (0,0) -- (3.,4.) node [right] {$c_s$};

  \fill[black] (1.5,1.5*4./3.) circle (0.05) node [above] {$P$};
  %% Other characteristics
  % stationary
  \draw (1.5,0.75*1.5) -- (1.5,1.5*4./3.);
  % fast plus (slope =+-0.25)
  \newcommand\px{1.5}
  \newcommand\py{1.5*4./3.}
  \draw (2.*\py-0.5*\px,1.5*\py-3.*\px/8.) node [right] {$c_f$}-- (1.5,1.5*4./3.) ;
  % fast minus
  ...
  % stationary
  \draw (1.5+\shift,1.5/4.) -- (\pxx+\shift,\pyy);
  % fast minus
  \draw (8.*\pyy/7.+5.*\pxx/7.+\shift,2.*\pyy/7.+10.*\pxx/56.) node [below right] {$c_f$}-- (\pxx+\shift,\pyy);
  % slow plus
  \draw (-12.0*\pyy/13.+16.*\pxx/13.+\shift,-3.*\pyy/13.+4.*\pxx/13.)node [below] {$c_s$} -- (\pxx+\shift,\pyy);
  % slow minus

...
Second, if the function $\psi_1^f$ vanishes at some point of the stress space, the prejection of the stress path in the $(\sigma_{11},\sigma_{12})$ plane is vertical according to the ODE \eqref{eq:sigSlow_n=e1}.
...
\begin{align}
  ...
  % & \tilde{A}_{12}^{ep}=\tilde{C}^{ep}_{1112} - \frac{\tilde{C}^{ep}_{1133}\tilde{C}^{ep}_{1233}}{\tilde{C}^{ep}_{3333}} = -\beta s_{11}s_{12}- \frac{\(\lambda - \beta s_{11}s_{33}\)\( -\beta s_{12}s_{33}\)}{\lambda + 2\mu - \beta s_{33}^2}
\end{align}
% \begin{equation*}
%   \tilde{A}_{12}^{ep} = \beta s_{12} \( s_{33}\frac{\lambda - \beta s_{11}s_{33}}{\lambda + 2\mu - \beta s_{33}^2} -s_{11}  \) =\beta s_{12} \frac{\lambda s_{33} - (\lambda + 2\mu)s_{11} }{\lambda + 2\mu - \beta s_{33}^2}  
% \end{equation*}
...
The other singularity $\tilde{A}^{ep}_{11}=\tilde{A}^{ep}_{22}$ is on the other hand, more complex to deal with.

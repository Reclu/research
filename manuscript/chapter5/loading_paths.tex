...
Second, if the function $\psi_1^s$ vanishes at some point of the stress space, the projection in the $(\sigma_{11},\sigma_{12})$ plane of the loading path followed inside a slow wave is vertical according to the ODE \eqref{eq:sigSlow_n=e1}.
...
Nevertheless, since the stress state $s_{12}=0$ also constitutes a singular point for plane stresses, the same remarks than the made for plane strains on the loading paths hold.

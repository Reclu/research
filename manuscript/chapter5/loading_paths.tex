...
First, considering the vector $\vect{n}=\vect{e}_1$, the functions satisfy the otrhogonality property: $\psi^s_1\psi^f_1=-1$.
Indeed, with the left eigenvectors of the acoustic tensor given in equation \eqref{eq:ch5_eigenvectAcc}, the product $\psi^s_1\psi^f_1$ reads:
\begin{equation*}
...
Moreover, the vector $\vect{n}=\vect{e}_2$ leads to the same result.
Therefore, it allows us to restrict the study to one function only for each vector $\vect{n}$, say $\psi_1^s$ and $\psi_2^s$.

Second, if the function $\psi_1^s$ vanishes at some point of the stress space, the projection of the stress path in the $(\sigma_{11},\sigma_{12})$ plane is horizontal according to the ODE \eqref{eq:sigSlow_n=e1}.
Conversely, if the inverse of $\psi_1^s\rightarrow \infty$, the loading path is veritcal in the $(\sigma_{11},\sigma_{12})$ plane.
...
Hence, if $A_{12} =0$ and $A_{11} \neq A_{22} $, one has $\psi^s_1 =0$ and $\psi^f_1 \rightarrow \infty $.
...
According to the ODEs of table \ref{tab:simpleWavesEquations}, the particular values of the \textit{loading functions} $\psi_i^{s,f}$ through the simple waves propagating in direction $\vect{e}_i$ for $i=\{1,2\}$, yield information about the loading paths in stress space.
...
Nevertheless, those terms depend results from products of the components of the elastoplastic tangent modulus and have complex expressions.
As a consequence, the $\alpha_{ij}$ are assumed to have non-zero values.
...
The case of plane stress is first considered by using the tangent modulus of equation \eqref{eq:elastoplastic_tangent}, so that the components of the acoustic tensor for $\vect{n}=\vect{e}_1$ read:
...
\begin{subequations}
  ...
  \end{alignat}
\end{subequations}
Subtracting equations \eqref{eq:DP_A11} and \eqref{eq:DP_A22}, one first has: $A_{11}^{ep}-A_{22}^{ep}= \lambda + \mu -\beta \(s_{11}^2-s_{12}^2\)$.
Hence, the equation $A_{11}^{ep}-A_{22}^{ep}=0$ admits a set of solutions in stress space.
...
Each solution is studied in more details hereinafter.
...
According to equations \eqref{eq:eigen_acc_DP1} and \eqref{eq:eigen_acc_DP2}, the eigenvalues of the acoustic tensor read:
\begin{align*}
...
\end{align*}
so that the characteristic speed of slow waves reduces to that of elastic shear waves of plane strain $c_s=c_2=\sqrt{\mu/\rho}$. 
...
Considering first the relation \eqref{eq:plane_strain_stress33} between stress components for plane strain, one has:
\begin{equation*}
...
\end{align*}
so that the celerity of fast waves identifies to that of elastic pressure wave of plane strain $c_f=\sqrt{(\lambda + 2\mu)/\rho}=c_1$.

%%%% n=e2
The same analysis can be carried out in the direction $\vect{n}=\vect{e}_2$ by consideringthe following acoustic tensor:
\begin{subequations}
  \begin{alignat}{1}
    \label{eq:DP_A11}
  ...
  \end{alignat}
\end{subequations}
in such a way that the characteristic speeds are:
\begin{subequations}
  \label{eq:eigen_acc_DP}
  \begin{alignat}{1}
    \label{eq:eigen_acc_DP1}
...
\begin{subequations}
  ...
  \end{alignat}
\end{subequations}
First, in order to ensure the hyperbolicity of the system, the component of the acoustic tensor also have to be defined, that is $\widetilde{C}^{ep}_{3333}\neq 0$. This condition leads to:
\begin{equation*}
...
\end{equation}
% leading to $\psi_2^s \rightarrow \infty$ as stated by equation \eqref{eq:first_root_psi2cp}.
% As a result, the conditions \eqref{eq:CP_roots_n=e2} yields slow and fast waves propagating in direction $\vect{n}=\vect{e}_2$, through which the loading paths are respectively horizontal and vertical in ($\sigma_{22},\sigma_{12}$) plane.

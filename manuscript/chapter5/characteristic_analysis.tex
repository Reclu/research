...
The removal of $\sigma_{33}$ from system \eqref{eq:ch5_quasilinear_normal} for both plane strains and plane stresses allows solving the problem in a two-dimensional setting.
...
Furthermore, one stationary wave associated to the zero eigenvalue of the Jacobian matrix, and whose left eigenvector satisfies equation \eqref{eq:ch5_null_eigen}, has to be added:
\begin{equation}
...
Figures \ref{fig:ch5_charac_method}\subref{subfig:slowWave} and \ref{fig:ch5_charac_method}\subref{subfig:fastWave} schematically illustrate the method for slow and fast simple waves in which the state is known along the head wave and is looked for at point $P$ lying on the tail wave. 
...
\begin{table}[h!]
  ...
  \caption{Summary of the ODE satisfied inside right-going slow and fast simple waves.}

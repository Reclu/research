\begin{tikzpicture}[scale=1.5]
  ...
  \draw[->, very thick,Blue] (3,3.75) -- (3,4.25) node [above] {$B^+A^+_{L/C}$}; % Top
  \draw[->, very thick,Blue] (3.75,3.) -- (4.25,3.) node [right] {$B^+A^+_{B/C}$}; % Right
  \draw[<-, very thick,Blue] (3.,2.25) -- (3.,1.75) node [below] {$B^+A^+_{BL/B}$}; % Bottom
  \draw[<-, very thick,Blue] (2.25,3.) -- (1.75,3.) node [left] {$B^+A^+_{BL/L}$}; % Left

...
% Test to explain this
% To understand what happends, consider the nodal velocity field resulting from the projection the Riemann-type initial conditions prescribed on material points. A regular distribution of particles in the mesh leads to a zero velocity at the central node after the mapping \eqref{eq:particles2nodes}, which is the correct solution. Next, initial conditions in stress and velocity yield null internal forces everywhere in the mesh for both USL and USF. As a consequence, the first solution of discrete equations does not alter the nodal velocity field. , and the discontinuous shape functions gradient so that the nodal velocity does not change after    
% "locking" of velocity 
% The symmetrical stress solution depicted in figures \ref{fig:US_diffusion}\subref{subfig:US_diffusion_10} and \ref{fig:US_diffusion}\subref{subfig:US_diffusion_25}, combined to the discontinuous gradient of shape functions, yield zero internal forces at the central node
% Furthermore, a skew-symmetric velocity profile leads, after the convective step, to a zero velocity at the central node, which is the correct solution (and never changes because of null internal forces).

This is not due to the computation of internal forces since the symmetrical stress solutions plotted in figures \ref{fig:US_diffusion}\subref{subfig:US_diffusion_10} and \ref{fig:US_diffusion}\subref{subfig:US_diffusion_25}, combined to the discontinuous gradient of shape functions, yield zero internal forces at the central node of the computational grid (equation \eqref{eq:int_forces}). Furthermore, a skew-symmetric velocity profile leads, after the convective step, to a zero velocity at the central node, which is the correct solution (and never changes because of null internal forces). Hence, the error most likely comes from the back-mapping of fields from the grid to material points. Moreover, this projection is known to be responsible for noise introduced in the numerical solution \cite{Mass_Flip}. On the other hand, the projection used in PIC is a simple interpolation of the updated nodal velocity:
\begin{equation}

...
Thus, the specific PK1 stress tensor $\rho_0\tens{\bar{\Pi}}=\tens{\Pi}$ is introduced so that every volume integrals of the weak are evaluated by treating particles as quadrature points:
\begin{equation}
...
\begin{equation}
  ...
  M_{ij} \vect{\dot{v}}^j = \vect{f}_{int}^i + \vect{f}_{ext}^i 
\end{equation}
Since the particles are considered as integration points, the diagonally lumped mass matrix is used in order to circumvent an eventual singularity. Hence, the mass matrix and forces vectors involved in the semi-discrete \eqref{eq:lagrangian_mpm_semi-discrete} equation read:
\begin{subequations}
  \begin{alignat}{1}
    \label{eq:lag_mass}
...
At last, the explicit time discretization of system \eqref{eq:lagrangian_mpm_semi-discrete} yields the discrete system that is solved at nodes to advance the solution from time increment $t^n$ to $t^{n+1}$:
\begin{equation}
...
Suppose that position and velocity vectors, as well as specific PK1 tensor, are stored at material points at time $t^n$ in the volume $\Omega_n$, that is: $\vect{x}^{p,n}$, $\vect{v}^{p,n}$ and $\tens{\bar{\Pi}}^{p,n}$ are known $\: \forall p=1,...,N_p$.
One first has to compute the mass matrix \eqref{eq:lag_mass} in the reference configuration which may be defined, as proposed in \cite{Love}, as the \textbf{the last computed} one, that is $\Omega_0\equiv \Omega_{n}$. This \textit{updated Lagrangian} formulation of the MPM bases the computation of the mass matrix on the shape functions evaluated at particle positions $\vect{x}^{p,n}$.
Alternatively, the reference configuration may be kept constant during simulations so that the mass matrix is computed once and for all. It is however worth noticing that such a \textit{total Lagrangian} formulation of the MPM is, to our knowledge, not met in the litterature.
...
Once internal as well as external forces are built from equations \eqref{eq:lag_fint} and \eqref{eq:lag_fext} respectively, acceleration and velocity are computed on the grid by successively solving systems \eqref{eq:lagrangian_mpm_semi-discrete} and \eqref{eq:lagrangian_mpm_discrete}. Than, according to FLIP mapping \eqref{eq:mp_velocity_update}, these fields areare used to update the material points kinematics (\textit{i.e.} $\vect{v}^{p,n+1}$ and $\vect{x}^{p,n+1}$).
Moreover, the gradient of shape functions provide the deformation gradient, based on the following explicit time discretization:
\begin{equation}
...
\end{equation}
so that the deformation gradient is updating in the updated Lagrangian formulation according to:
\begin{equation}

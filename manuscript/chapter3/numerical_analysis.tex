...
\end{equation}
% Moreover, nodal interface flux $\hat{f}_i^{*}$ results from the integration of Godunov's flux along the edges it belongs to, which for equation \eqref{eq:2D_model_equation} are:
% \begin{equation}
%   \label{eq:2d_Godunov_fluxes}
%   f^{*}= c_n q_U = \underbrace{c_nq_D}_{f_N(q_D)} - \underbrace{c_n (q_D -q_U)}_{A^{+}_{U/D}} 
% \end{equation}
% where $c_n$ is the speed in the direction normal to the edge, $q_U$ and $q_D$ are state vectors obtained by averaging nodal values in Upwind and Downwind cells $U$ and $D$ respectively, and $A_{U/D}^+$ is the right-going fluctuation.
% The CTU is adopted by accounting for the transverse corrections based on those fluctuations according to equation \eqref{eq:transverse_fluctuations}:
% \begin{equation}
%   \label{eq:2D_transverse_corrections}
%   B^+A^+_{U/D}=c_t c_n (q_D -q_U)
% \end{equation}
...
Figure \ref{fig:2Dmesh} shows an exemple of such transverse corrections resulting from fluctuations between Bottom ($B$), Left ($L$), Bottom Left ($BL$) elements and a given cell $C$. A numbering of nodes and interfaces within a cell can also be in the figure.
\begin{figure}[h!]
  \centering
...
Introduction of the convective phase where $Q_I=\rho \bar{Q}_I= \frac{m_I N_p}{\Delta X^2}\bar{Q}_I$ with no sum on $I$:
\begin{align}

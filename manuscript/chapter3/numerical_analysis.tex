...
\end{equation}
The stability of the scheme is thus ensured by using the lowest CFL number satisfying \eqref{eq:stability}. \thomas{Note however that the use of the triangle inequality leads to a more severe constraint so that in practice, the Courant number may be set to higher values than that resulting from the solution of \eqref{eq:stability}.} 
...
% \begin{figure}[h!]
%   \centering
%   {\input{chapter3/pgfFigures/convS_2ppclowCFL.tex}}
%   {\input{chapter3/pgfFigures/convS_6ppclowCFL.tex}}
%   {\input{chapter3/pgfFigures/convS_10ppclowCFL.tex}}
%   {\input{chapter3/pgfFigures/convS_20ppclowCFL.tex}}
%   \caption{Evolution of the $L^2$ error on stress with regard to the number of particles lying in every cells CFL set to 0.5 for DGMPM.}
%   \label{fig:convergence_stress}
% \end{figure}
\begin{table}[h!]
  \centering
...
Since the DGMPM aims at capturing non-regular solutions (discontinuities), the first-order accuracy it exhibits appears better suited than a higher-order one since the achievable accuracy for such solutions is at most one \cite[p.149]{Leveque}. %that would introduce additional regularity in the approximation.

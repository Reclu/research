...
\end{align}
% \begin{align}
%   & S_1(\xi,\eta) = \frac{(1-\xi)(1-\eta)}{4} \quad ; \quad S_2(\xi,\eta) = \frac{(1+\xi)(1-\eta)}{4} \\
%   & S_3(\xi,\eta) = \frac{(1+\xi)(1+\eta)}{4}\quad ; \quad S_4(\xi,\eta) = \frac{(1-\xi)(1+\eta)}{4}
% \end{align}
\begin{figure}[ht]
  \centering
...
\begin{equation}
  \begin{aligned}
    & K_{ij}^X = \sum_\beta \drond{S_{i\beta}}{X}m_\beta S_{j\beta}=\frac{2}{\Delta X}\sum_\beta\drond{S_{i\beta}}{\xi}m_\beta S_{j\beta} =\frac{2}{\Delta X}\sum_\beta\nabla_\xi S_{i\beta} m_\beta S_{j\beta}\\
    &K_{ij}^Y = \sum_\beta\drond{S_{i\beta}}{Y}m_\beta S_{j\beta}=\frac{2}{\Delta X}\sum_\beta\drond{S_{i\beta}}{\eta}m_\beta S_{j\beta} =\frac{2}{\Delta X}\sum_\beta\nabla_\eta S_{i\beta}m_\beta S_{j\beta}
  \end{aligned}
\end{equation}
and subsequently:
\begin{equation}
  ...
  \begin{aligned}
    & \frac{K_{ij}^X}{M_i^L}  =  \frac{2}{\Delta X} \frac{\sum_\beta\nabla_\xi S_{i\beta}  S_{j\beta}}{\sum_\gamma  S_{i\gamma}}=\frac{2}{\Delta X} \frac{k^X_{ij}}{\sum_\gamma  S_{i\gamma}} \\
    & \frac{K_{ij}^Y}{M_i^L} = \frac{2}{\Delta X} \frac{\sum_\beta\nabla_\eta S_{i\beta} S_{j\beta}}{\sum_\gamma S_{i\gamma}} = \frac{2}{\Delta X}  \frac{k^Y_{ij}}{\sum_\gamma  S_{i\gamma}}
  \end{aligned}
\end{equation}
...
\begin{equation}
  ...
  \begin{aligned}
    & a\frac{K_{ij}^X}{M_i^L}\bar{q}^n_j  = \sum_\beta \bar{Q}^n_\beta} \frac{2}{\Delta X} \frac{a k^X_{ij}}{\sum_\gamma  S_{i\gamma}\sum_\mu S_{j\mu}}S_{j\beta} \\
    & b\frac{K_{ij}^Y}{M_i^L}\bar{q}^n_j = \sum_\beta \bar{Q}^n_\beta}\frac{2}{\Delta X}  \frac{b k^Y_{ij}}{\sum_\gamma  S_{i\gamma}\sum_\mu S_{j\mu}}S_{j\beta}
  \end{aligned}
\end{equation}
...
\end{align}
where $q^{C,n}_i= \rho \bar{q}^{C,n}_i$ is the value at time step $n$ and node $i$ of cell $C$ in which the uniform mass density is defined as $\rho = \frac{N_p^{C} m^C}{\Delta X^2}$ with $N_p^C$, the total number of particles in that element, and $m^C$ the mass they carry. Thus, introduction of the convective phase yields the following expressions for interface fluxes:
\begin{align}
  & f^{(1)} = \sum_{\beta=1}^{N_p}\bar{q}_\beta^n\frac{b N^C_p m^C }{2\Delta X^2} \[  \(\frac{S_{3\beta}^{B} }{\sum_P S_{3P}^{B}} + \frac{S_{4\beta}^{B}}{\sum_PS_{4P}^{B}}\) - a  \frac{\Delta t}{2\Delta X}\(\frac{S_{1\beta}^{B}}{\sum_P S_{1P}^B} + \frac{S_{4\beta}^{B}}{\sum_PS_{4P}^{B} }-\frac{S_{2\beta}^{BL}}{\sum_P S_{2P}^{BL}} - \frac{S_{3\beta}^{BL}}{\sum_PS_{3P}^{BL}}\) \]\\
  & f^{(2)} = \sum_{\beta=1}^{N_p} \bar{q}_\beta^n\frac{a N^C_p m^C }{2\Delta X^2} \[  \(\frac{S_{2\beta}^{C}}{\sum_P S_{2P}^{C}} + \frac{S_{3\beta}^{C}}{\sum_P S_{3P}^{C}} \)- b \frac{\Delta t}{2\Delta X}\(\frac{S_{1\beta}^{C}}{\sum_P S_{1P}^{C}} + \frac{S_{2\beta}^{C}}{\sum_P S_{2P}^{C}}-\frac{S_{3\beta}^{B}}{\sum_P S_{3P}^{B}} -\frac{S_{4\beta}^{B}}{\sum_P S_{4P}^{B}}\) \]\\
  & f^{(3)} =\sum_{\beta=1}^{N_p}\bar{q}_\beta^n\frac{b N^C_p m^C}{2\Delta X^2} \[  \(\frac{S_{3\beta}^{C}}{\sum_P S_{3P}^{C}} + \frac{ S_{4\beta}^{C}}{\sum_P S_{4P}^{C}}\) - a  \frac{\Delta t}{2\Delta X}\(\frac{S_{1\beta}^{C}}{\sum_P S_{1P}^{C}} + \frac{S_{4\beta}^{C}}{\sum_P S_{4P}^{C}}-\frac{S_{2\beta}^{L}}{\sum_P S_{2P}^{L}} - \frac{S_{3\beta}^{L}}{\sum_P S_{3P}^{L}}\) \]\\
  & f^{(4)} = \sum_{\beta=1}^{N_p}\bar{q}_\beta^n\frac{a N^C_p m^C }{2\Delta X^2}  \[  \(\frac{S_{3\beta}^{L}}{\sum_P S_{3P}^{L}} + \frac{ S_{4\beta}^{L}}{\sum_P S_{4P}^{L}}\) - b \frac{\Delta t}{2\Delta X}\(\frac{S_{1\beta}^{L}}{\sum_P S_{1P}^{L}} + \frac{S_{2\beta}^{L}}{\sum_P S_{2P}^{L}}-\frac{S_{3\beta}^{BL}}{\sum_P S_{3P}^{BL}} - \frac{S_{4\beta}^{BL}}{\sum_P S_{4P}^{L}}\)\]
\end{align}
written for simplicity:
\begin{equation}
  \label{eq:interface_flux_mapped}
  f^{(i)}=\sum^{N_p}_\beta \bar{Q}_\beta^n \frac{c_n N^C_p m^C}{2\Delta X^2}\[ \phi_\beta^{(i)} + \phi_\beta^{(i),T} \]
\end{equation}
In the latter expressions, $\phi^{(i)}$ is devoted to normal contributions while $\phi^{(i),T}$ stands for transverse corrections at interface $(i)$.
Fluxes contribute to nodes through the boundary integrals of the discrete form:
\begin{equation}
  \hat{f}_i^{*} = \int_{\Gamma_C} S_i(\vect{X}) f^*_N  \: d\Gamma
\end{equation}
where $f^*_N$ is the intercell flux in the direction of the outward normal vector to the boundary. However, interface fluxes computed so far are based on the stream directions so that $f^{(1)}$ and $f^{(4)}$ must be counted negatively in integrals in order to fit outgoing fluxes. Hence, nodal interface fluxes can be computed analytically by using parent coordinates and read for cell $C$ \cite[p.494]{Trangenstein}: 
\begin{equation}
  \hat{f}_i^{*} =  -\int_{X^C_1}^{X_2^C} S_i(\vect{X}) f^{(1)}  dX + \int_{Y^C_2}^{Y_3^C} S_i(\vect{X}) f^{(2)}  dY +\int_{X^C_2}^{X_3^C} S_i(\vect{X}) f^{(3)}  dX -\int_{Y^C_1}^{Y_4^C} S_i(\vect{X}) f^{(4)}  dY 
\end{equation}
so that edges $(1)$ and $(4)$ corresponds to ingoing flux regions while $(2)$ and $(3)$ are outgoing boundaries of the element. Those integrals can be computed analytically by mapping them to parent coordinates leading thus to:
\begin{subequations}
  \begin{alignat}{4}
    &\hat{f}_1^{*} = -&\frac{\Delta X}{2}\[f^{(1)} +f^{(4)}\] \quad;\quad &\hat{f}_2^{*} = -&\frac{\Delta X}{2}\[f^{(1)} -f^{(2)}\] \\
    &\hat{f}_3^{*} = &\frac{\Delta X}{2}\[f^{(2)}+ f^{(3)}\]  \quad;\quad &\hat{f}_4^{*} = &\frac{\Delta X}{2}\[f^{(3)} -f^{(4)}\] 
  \end{alignat}
\end{subequations}
Those expression are then condensed by introducing the middle point of edges with coordinates denoted by $\vect{X}^{(i)}_{1/2}$, at which the shape functions are:
\begin{equation*}
  S_i(\vect{X}^{(j)}_{1/2}) =
  \left\lbrace
  \begin{aligned}
  ...
  \right.
\end{equation*}
In addition, the component of the outward normal vector of the edge $N^{(i)}_X$ and $N^{(i)}_Y$ are considered in order to take into account the different signs of intercell fluxes in the Cartesian grid. One thus writes:
\begin{equation}
  \hat{f}_i^{*}= \frac{\Delta X}{2}\sum_j^{\text{edges}} 2S_{i}(\vect{X}^{(j)}_{1/2})\(N^{(j)}_X + N^{(j)}_Y\) f^{(j)}
\end{equation}
which, combined to equation \eqref{eq:interface_flux_mapped} leads to:
\begin{equation}
  \hat{f}_i^{*}= \sum^{N_p}_\beta \bar{Q}_\beta^n \sum_j^{\text{edges}} S_{i}(\vect{X}^{(j)}_{1/2})\(aN^{(j)}_X + bN^{(j)}_Y\)  \frac{ N^C_p m^C}{2\Delta X}\[ \phi_\beta^{(i)} + \phi_\beta^{(i),T} \]
\end{equation}
These terms are divided by the lumped mass matrix in the discrete form:
\begin{equation}
  \label{eq:nodal_fluxes}
  \frac{\hat{f}_i^{*}}{M_i^L}=\sum_\beta \frac{\bar{Q}_\beta^n}{\sum_\mu S_{i\mu}}   \sum_{j=1}^{\text{edges}} S_{i}(\vect{X}^{(j)}_{1/2}) \(aN_X^{(j)}+bN^{(j)}_Y\)\frac{N_p^C }{2 \Delta X}\[\phi_\beta^{(j)} + \phi_\beta^{(j),T}\] 
\end{equation}
...
\begin{equation}
  ...
  \begin{split}
    \bar{Q}_\alpha^{n+1}= \sum_{i=1}^{4E} &\frac{\sum_{\beta=1}^{N_p}\bar{Q}_\beta^nS_{i\alpha}}{\sum_\mu S_{i\mu}}  \left\lbrace \vphantom{\sum_{j=1}^{\text{edges}} } S_{i\beta} +  2\frac{\Delta t}{\Delta X}  \[ a k^X_{ij} + b k^Y_{ij} \]\frac{S_{j\beta}}{\sum_\gamma S_{i\gamma}} \right.\\ - & \frac{\Delta t}{2\Delta X}\left.\sum_{j=1}^{\text{edges}} S_{i}(\vect{X}^{(j)}_c) N_p^C(aN_X^{(j)}+bN^{(j)}_Y)\[\phi_\beta^{(j)} + \phi_\beta^{(j),T}\] \right\rbrace
  \end{split}
\end{equation}
Recall that in the two-dimensional scheme equation \eqref{eq:2Dscheme_equation}, transverse contributions $\phi_\beta^{(j),T}$ depend on $\Delta t$, providing thus second order corrections. Equation \eqref{eq:2Dscheme_equation} can also be rewritten as:
\begin{equation}

...
\end{equation}
We see that the eigenvalues can become complex, leading to a loss of hyperbolicity of the problem. Nevertheless, we assume in what follows that $F>\sqrt{\frac{1}{3}}$.
...
\end{equation*}
with is not zero since $F>\sqrt{\frac{1}{3}}$. The two characteristic fields are hence genuinely non-linear. The dependence of characteristic speeds on the deformation gradient make the structure of the solution dependent on initial data. Indeed, assuming intitial conditions such that $F_L<F_R$ yields the following characteristic speeds in the $(x,t)$ plane:

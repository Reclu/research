...
\end{equation*}
The system of ODE along the characteristic curves is then:
\begin{equation}
...
\end{equation*}
with neglected body forces. \todo{rewrite the good quasilinear form}The specialization of the left eigensystem \eqref{eq:eigen_left_stress}-\eqref{eq:eigen_left_velo} with left eigenvectors $\Lcb^k=\[v^k,F^k\]$ and the acoustic tensor \eqref{eq:SVK_tangent}, yields the following left characteristic fields:
\begin{equation}
  \label{eq:SVK_charac_fields}
  \lambda_{1,2}=\pm \sqrt{\frac{\lambda+2\mu}{2\rho_0}\(3F^2-1\) } \quad ; \quad \Lcb^p=\[\rho_0 \lambda_p \:,\: -1\] 
\end{equation}

...
The basic assumption underlying rate-independent plasticity is that admissible thermodynamic forces lie within the elastic domain \eqref{eq:elastic_convex}. Furthermore, the mechanical dissipation is supposed to be maximum according to \textit{Hill's principle}. The optimization procedure of the mechanical dissipation under the constraint $f(\tens{\sigma},\Acb)\leq 0$ leads to the following flow rule:
\begin{equation}
  \label{eq:flow_hardening_rules}
  \tens{\dot{\eps}}^p=\drond{\Phi}{f}\drond{f}{\tens{\sigma}} \quad ; \quad \dot{\Vcb} = \drond{\Phi}{f}\drond{f}{\Acb}
\end{equation}
where $\dot{p}=\drond{\Phi}{f}$ is the \textit{plastic multiplier} that ensures the non-positiveness of $f$ and gives the instensity of the plastic flow. The plastic multiplier must obey the following \textit{K{\"u}hn-Tucker compatibility conditions}:
\begin{equation}
...
On the other hand, viscoplastic admissible thermodynamic forces may lie out of the elastic domain \eqref{eq:elastic_convex} (\textit{i.e. $f(\tens{\sigma},\Acb) > 0$ is possible}). In that sense, viscoplasticity can be seen as a regularization of rate-independent plasticity (penalization function based on the positive part \cite[p.107]{Simo}consequences in terms of dissipation pseudo-potential, etc.). 

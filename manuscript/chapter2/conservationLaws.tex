...
\item \textbf{contraint} minimization $\rightarrow$ \eqref{eq:flow_hardening_rules}, where $\dot{p}$ determined with the consistancy condition $\dot{f}=0$.We thus see that the plastic flow is normal to the yield function 
\item Conditions de charge--décharge: K{\"u}hn-tucker complementarity conditions $\dot{p}\geq 0 ; f\leq 0 ; \dot{p}f=0$
\end{itemize}
...
Seen as a regularization of plasticity \cite[p.105]{Simo}, relaxing the constraint $f=0$ in the minimization process Perzina model
\begin{itemize}
\item modules tangent (Tout à la fin, en sous-section. S'intéresser au taux de variation de la contrainte)
\end{itemize}

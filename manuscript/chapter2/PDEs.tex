A wide variety of physical phenomena are modeled in the space-time domain by Partial Differential Equations (PDEs) of different types. The purpose of this section is to review generalities about PDEs and suited strategies depending on the natures of equations so that solutions can be derived.
...
% We prescribe initial values of $u$ along a curve $\Cscr_0$ in the $(x,t)$ plane defined as $\varphi(x,t)=const$. The set $\varphi(x,t),u\(\varphi(x,t)\)$ thus defines
...
However, the following statement holds \cite[Chapter~1]{Courant}:
\begin{theorem}
...
\end{theorem}
% With these considerations, we see from theorem \ref{th:integral_surface_generated} that a characteristic curve is tangent at every point to all the integral surfaces. In other words, an infinity of integral surfaces cross one characteristic curve.
...
\end{equation*}
%We are then left with two families of curves in the $(x,t)$ plane due to the dependence of the coefficient matrices of the system on independent variables.
Those curves are drawn in figure \ref{fig:exampleCharac}\subref{subfig:curve_lines} for several values of parameters $c_1$ and $c_2$.
\end{example}
\begin{example}
  \label{ex:charac2}
...
\end{equation*}
Unlike example \ref{ex:charac1}, coefficient matrices do not depend on independent variables, leading to characteristic straight lines in the $(x,t)$ plane (see \ref{fig:exampleCharac}\subref{subfig:straight_lines}).
\end{example}
\begin{figure}[h]
  \centering
  \subfloat[Example \ref{ex:charac1}: $\lambda_{1,2}=\pm 1/x$\label{subfig:curve_lines}]{\input{chapter2/pgfFigures/exampleCharac}} \qquad
  \subfloat[Example \ref{ex:charac2}: $\lambda_{1}=1 \:\text{and} \: \lambda_2=1/2$\label{subfig:straight_lines}]{\input{chapter2/pgfFigures/exampleCharac_const}}
...
\end{equation}
%Integration of equations \eqref{eq:PDEs_ODEs} yields a set of \textit{integral curves}.
...
%Although the considerations made so far remain very general, in what follows we will see that dynamic problems in solid dynamics can be written as first order quasi-linear systems that can be solved by using the method of characteristics. 

...
\end{equation}
one gets the following relation between the partial derivatives of $u$ on the initial curve:
\begin{equation*}
...
\begin{equation}
  ...
  (a - b\ddroit{x}{t})u_x = c - b\ddroit{u}{t}\lvert_{t\in\Cscr_0}
\end{equation}
...
\begin{equation}
  ...
  \( \Absf^x - \lambda \Absf^t \) \vect{\Uc}_x + \Scb + \Absf^t\ddroit{\Ucb}{t}\lvert_{t\in\Cscr_0} = \vect{0}
\end{equation}
where:
\begin{equation}
  ...
  \lambda=\ddroit{x}{t}
\end{equation}
The Cauchy problem admits a unique solution $\vect{\Uc}_x$ along $\Cscr$ if the determinant of the system does not vanish, that is:
\begin{equation}

...
An unique solution of equation \ref{eq:1st_order_pde} may be found by solving the initial value problem. A curve $\Cscr$ is defined in the space $(x,t,u)$ by means of a parameter $\xi$ along which we are given initial conditions that do not need to be analytic. We assume that $x_\xi^2 + t_\xi^2 \neq 0$ and that $x(\xi)$, $t(\xi)$ and $u(\xi)$ are continuously differentiable, and that $\Cscr$ has a unique projection $\Cscr_0$ on the $(x,t)$ plane. \textbf{With suitable initial conditions, the only solution of equation \ref{eq:1st_order_pde} is the one that contains $\Cscr$}. %or is it the definition of characteristic curve ?
...
\end{equation}
%then with theorems \ref{th:integral_surface_generated} and \ref{th:charac_in_integral_surface}, an unique integral surface is generated by the set of characteristic curves built along the initial curve $\Cscr$.
...
In order to solve this initial value problem, we first draw characteristic curves through each point of $\Cscr$ by means of equation \ref{eq:charac_curve} and a second parameter $\eta$. We are hence left with a family of characteristic curves:
\begin{equation}
  \label{eq:2}

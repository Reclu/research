...
\end{equation*}
which, according to equation \ref{eq:monge_axis}, corresponds to the partial differential equation \ref{eq:1st_order_pde}. If however, $J=0$ along the initial curve, $\Cscr$ is a characteristic curve and infinitely many integral surfaces contain it, which involves that Cauchy problem has infinitely many solutions\todo{ensure uniqueness}. These concepts extend to the case of first order non-linear PDEs with more than two independent variables \cite[Chapter~2]{Courant}.
...
\todo{see \cite[p.424]{Courant}, notion of free curves if a matrix is non-singular}
\begin{equation}

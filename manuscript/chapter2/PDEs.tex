...
The Cauchy problem can be formulated as the one of finding a surface $u(x,t)$ that contains $\Cscr$ and satisfies \eqref{eq:1st_order_pde}. One thus seeks the derivatives of $u$ along the initial curve. Since $u=u(\xi)$ along $\Cscr$, one can write:
\begin{equation*}
...
\end{equation}
Such a problem is called \textit{hyperbolic}. \textit{Hyperbolicity} can be formulated for first order systems as the requirement for a quasi-linear system of dimension $I$ \todo{regardless of the order?} to possess $I$ real eigenvalues associated to $I$ linearly independent eigenvectors. According to the definition \eqref{eq:lambda_slope}, it appears that the curves $\Cscr_0$ are characteristic curves which slopes in the $(x,t)$ plane are $-\lambda^k$.\todo{Link it to the concept developed in scalar case}

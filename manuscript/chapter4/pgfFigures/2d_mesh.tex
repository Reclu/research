\begin{tikzpicture}
  ...
  \foreach \x in {0.,1.,...,3.} 
  \draw (\x,0) -- (\x,3);
  \draw (3.75,0.75) rectangle (7.5,2.25);
  \fill (4.2,1.) circle (0.05) node [right] {$\: \: \text{Nodes / particles}$};
  \draw[dashed,gray] (4.,1.5) -- (4.4,1.5) node [right] {$\color{black} \text{Grid 1}$};
  \draw[dashed,very thick,Red] (4.,2.) -- (4.4,2.) node [right] {$\color{black} \text{Grid 2}$};
\end{tikzpicture}

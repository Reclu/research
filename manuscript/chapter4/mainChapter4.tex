...
In addition, linear isotropic or kinematic hardenings of modulus $C$ and tensile yield stress $\sigma^y$ are assumed for plastic evolutions, along with the viscosity $\eta$ and sensitivity $n$ for viscous ones.
\begin{table}[h!]
  \centering
...
As discussed in section \ref{subsec:hpp_planewave}, elastoplastic problems can be solved as the stiff limit of elastoviscoplastic ones, providing that suitable ODEs integrators are employed. %at the cost of additional computational effort.

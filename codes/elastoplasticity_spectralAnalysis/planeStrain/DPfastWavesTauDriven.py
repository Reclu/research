...
def loadingPath(dtau,sigma,sigman,E,nu,h,lamb,mu,beta,tangent):
    ...
    w1=eigens[1][0];w2=eigens[1][1]
    psi11=-w2/w1
    psi22=(w2*alpha11/w1-alpha12)/alpha22
    ## Plastic strain part
...
for k in range(len(sig22)):
    #for k in range(len(sig22)):
    s22=sig22[k]
    print "Initial yield surface"
    sigM=1.05*np.max(sig[:,k])
    tauM=0.#1.05*np.max(tau[:,k])
    ## For each value of sig22 trace the loading paths given by psif from yield surface to an arbitrary shear stress level
    ...
    for s,i in enumerate(frames):
        ...
        sig0=sig[i,k]
        tau0=tau[i,k]
        plt.plot(sig[:,k],tau[:,k],'k')
        plt.plot([0,sig0],[0,tau0],'ro')
        plt.grid()
        plt.show()
        ...
        #rFast = ode(computePsiFast).set_integrator('vode',method='bdf',order=5)
        rFast = ode(computePsiFast).set_integrator('vode',method='adams',order=12)
        #rFast = ode(computePsiFast).set_integrator('dopri5')
        
        rFast.set_initial_value(np.array([SIG11[0,s,k],SIG22[0,s,k]]),TAU[0,s,k]).set_f_params(sig33,lamb,mu,beta,tangent)
        ...
        for j in range(Niter-1):
            # rFast.set_f_params(np.array([SIG11[j,s,k],SIG22[j,s,k]]),SIG33[j,s,k],lamb,mu,beta,tangent)
            ...
            sigman=np.array([SIG11[j+1,s,k],TAU[j+1,s,k]*np.sqrt(2.),SIG22[j+1,s,k],SIG33[j+1,s,k]])
            # Plastic update
            ...
            rcf2[j+1,s,k]=computeSpeed(sigma,lamb,mu,beta,tangent)
        #plt.plot(TAU[:,s,k],SIG11[:,s,k])
        plt.plot(np.arange(0,Niter,1),LodeAngle_F[:,s,k])
        plt.grid()
        plt.show()
        ...
        if k==1:
            fileName=path+'fastWave_DevPlane_frame'+str(s)+'_StressValue'+str(k)+'.pgf'
            dico={"xlabel":r'$\sigma_1$',"ylabel":r'$\sigma_2$',"zlabel":r'$\sigma_3$'}
            #export2pgfPlot3D(fileName,eigsigS[0:-1:Niter/100,s,k,0],eigsigS[0:-1:Niter/100,s,k,1],eigsigS[0:-1:Niter/100,s,k,2],dico)
    ...
    if plot_path :
        ...
        for p,i in enumerate(frames):
            ...
            if updated_criterion :
                ...
                s22min=(sig0*(1.+2.*nu-2.*nu**2)  +E*Epsp33[-1,p,k]*(2.*nu-1.) +E*Epsp33[-1,p,k]*(2.*nu-1.)-np.sqrt(Delta))/(2.*(nu**2-nu+1.))
                s22=np.linspace(s22min,s22max,Samples)
    ...
    name1='DPfastTau_sig11_tau'+str(k)+'Tau.tex'
    name2='DPfastTau_sig22_tau'+str(k)+'Tau.tex'
    name3='DPfastTau_deviator'+str(k)+'Tau.tex'

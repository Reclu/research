...
#for k in range(len(sig22)):
    ...
    for s,i in enumerate(frames):
        ...
        export2pgfPlotFile(fileName,np.array([TAU[ranging,s,k],SIG11[ranging,s,k],SIG22[ranging,s,k],rcs2[ranging,s,k],LodeAngle_S[ranging,s,k],eigsigS[ranging,s,k,2]+eigsigS[ranging,s,k,1]+eigsigS[ranging,s,k,0],eigsigDevS[ranging,s,k,2],MaxCrit[ranging,s,k]]),'sigma_12','sigma_11','sigma_22','p','Theta','hydro','s3','maxCrit')
        ...
        # export2DTeXFile('LodeAngle_s22.tex',np.array([LodeAngle_S[ranging,0,k],LodeAngle_S[ranging,1,k],LodeAngle_S[ranging,2,k],LodeAngle_S[ranging,3,k]]),r'$\Theta ()$',r'$\sigma_{11} \: (Pa)$','',np.array([SIG22[ranging,0,k],SIG22[ranging,1,k],SIG22[ranging,2,k],SIG22[ranging,3,k]]),legend)
    ...
    if plot_path :
        ...
        for p,i in enumerate(frames):
            plast=plast_S[iteration[p,k]-1,p,k]
            sig0=sig[-1-i,k]
            ...
            if updated_criterion :
                ...
                sig0=SIG11[iteration[p,k],p,k]
                maxCrit=0.5*(sig0*(2.*nu**2-2.*nu-1.)+E*Epsp33[iteration[p,k],p,k]*(1.-2.*nu))/(nu-nu**2-1.)
                Delta=(4.*(nu**2-nu+1.)*(sigy+H*plast)**2- 3.*(E*Epsp33[iteration[p,k],p,k]+(1.-2.*nu)*sig0)**2)
                s22max=(sig0*(1.+2.*nu-2.*nu**2) +E*Epsp33[iteration[p,k],p,k]*(2.*nu-1.) +np.sqrt(Delta))/(2.*(nu**2-nu+1.))
                s22min=(sig0*(1.+2.*nu-2.*nu**2)  +E*Epsp33[iteration[p,k],p,k]*(2.*nu-1.) -np.sqrt(Delta))/(2.*(nu**2-nu+1.))
                s22=np.linspace(s22min,s22max,Samples)
                delta=(-(E*Epsp33[iteration[p,k],p,k])**2 +E*Epsp33[iteration[p,k],p,k]*(sig0+s22)*(2.*nu-1) + sig0*s22*(2.*nu+1.-2.*nu**2) + (sig0**2+s22**2)*(nu-nu**2-1.)+ (sigy+H*plast)**2)/3.
                if (np.abs(delta)<10.).any() : delta=np.abs(delta)
                s12=np.sqrt(delta)
                ...
                Delta=(4.*(nu**2-nu+1.)*(sigy+H*plast)**2- 3.*(E*Epsp33[iteration[p,k],p,k]+(1.-2.*nu)*s22)**2)
                sigMax=(s22*(1.+2.*nu-2.*nu**2) +E*Epsp33[iteration[p,k],p,k]*(2.*nu-1.) +np.sqrt(Delta))/(2.*(nu**2-nu+1.))
                sigMin=(s22*(1.+2.*nu-2.*nu**2) +E*Epsp33[iteration[p,k],p,k]*(2.*nu-1.) -np.sqrt(Delta))/(2.*(nu**2-nu+1.))
                s11=np.linspace(sigMin,sigMax,Samples)
                maxCrit=0.5*(s22*(2.*nu**2-2.*nu-1.)+E*Epsp33[iteration[p,k],p,k]*(1.-2.*nu))/(nu-nu**2-1.)
                delta=(-(E*Epsp33[iteration[p,k],p,k])**2 +E*Epsp33[iteration[p,k],p,k]*(s11+s22)*(2.*nu-1) + s11*s22*(2.*nu+1.-2.*nu**2) + (s11**2+s22**2)*(nu-nu**2-1.)+ (sigy+H*plast)**2)/3.
                if (np.abs(delta)<10.).any() : delta=np.abs(delta)
                s12=np.sqrt(delta)
                ## export to pgf file
            ...
            ax2.plot(SIG22[:iteration[p,k],p,k],TAU[:iteration[p,k],p,k],color=col[p],lw=2.5)
            ax4.semilogy(TAU[:iteration[p,k],p,k],plast_S[:iteration[p,k],p,k],color=col[p],lw=2.5)
            #ax4.plot(TAU[:iteration[p,k],p,k],plast_S[:iteration[p,k],p,k],color=col[p],lw=2.5)
            #ax3.plot(eigsigDevS[:iteration[p,k],p,k,0],eigsigDevS[:iteration[p,k],p,k,1],eigsigDevS[:iteration[p,k],p,k,2],color=col[p],lw=2.5)
            ax3.plot(eigsigS[:iteration[p,k],p,k,0],eigsigS[:iteration[p,k],p,k,1],eigsigS[:iteration[p,k],p,k,2],color=col[p],lw=2.5)

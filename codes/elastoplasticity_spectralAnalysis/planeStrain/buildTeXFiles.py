...
def buildTeXFiles(names,pgfFiles,xlabels,ylabels,zlabels,subtitle,srcX,srcY):
    ...
    for i,nom in enumerate(names):
        ...
        couleur=['Orange','Blue','Red','Purple','Green','Duck']
   		    ...
   		    if rando[2:6]=='slow':
                ...
                for j,name in enumerate(pgfFiles[i][k]):
                    ...
                    if name[31:31+6]=='_yield': ##  yield surface
                        ...
                        TeXFile.write(r'\addplot[mesh,point meta = \thisrow{p},very thick,no markers] table[x='+str(srcX[k])+',y='+str(srcY[k])+'] {chapter5/pgfFigures/'+name+r'} node[above,black] {$\textbf{'+str(j+1)+'}$};')
                    TeXFile.write('\n')

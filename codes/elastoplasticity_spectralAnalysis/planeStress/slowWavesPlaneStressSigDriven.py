...
def computePsiSlow(sigma,sig,sig33,lamb,mu,beta,tangent):
    # sig12 driven
    n1=1.;n2=0.
    sig11=sig;sig12=sigma[0];sig22=sigma[1]
...
def integrateODE(dsig,sig0,tau0,sig22_0,sig33,lamb,mu,beta,tangent):
    ...
    for i in range(sub_steps):
        ...
        ## R = s^{n+1} - s^{n} - RHS
        R=lambda x: x - sigma - dSIG*(theta*computePsiSlow(x,sig0+dSIG,sig33,lamb,mu,beta,tangent)+(1.0-theta)*computePsiSlow(sigma,sig0,sig33,lamb,mu,beta,tangent))
        #pdb.set_trace()

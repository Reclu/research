...
def export2DTeXFile(fileName,xFields,xlabel,ylabel,subtitle,yfields,*kwargs):
    ...
    couleur=['Red','Orange','Blue','Purple','Green','black','black','Yellow']
...
print 'Computing DGMPM'
execfile('dgmpm/barEP_EPsolver.py', DGMPM2)
...
execfile('dgmpm/barEP_EPsolver.py', DGMPM3)
...
for i,n1 in enumerate(frames):
    ...
    ax1.plot(MPM["pos"][:,n1],MPM["sig"][:,2*n1],'g',lw=2.,ms=4.,label='usl 1ppc')
    ax1.plot(DGMPM["pos"][:,n1],DGMPM["sig"][:,n1],'r',lw=2.,ms=4.,label='dgmpm 1ppc')
    ax1.plot(MPM2["pos"][:,n1],MPM2["sig"][:,2*n1],'g--',lw=2.,ms=4.,label='usl 2ppc')
    ax1.plot(DGMPM2["pos"][:,2*n1],DGMPM2["sig"][:,2*n1],'b',lw=2.,ms=4.,label='dgmpm 2ppc')
    ax1.plot(DGMPM3["pos"][:,n1],DGMPM3["sig"][:,n1],'y',lw=2.,ms=4.,label='dgmpm 2ppc (RK2)')
    ax1.plot(DGMPM["pos"][:,n1],-np.sign(v0)*Sexact,'k',lw=1.,ms=4.,label='exact')
    ax1.set_title('Stress at time t='+str(time)+' s.')
    ax1.set_xlabel('x (m)')
    ax1.set_ylabel(r'$\sigma (Pa)$')

    ax2.plot(MPM["pos"][:,2*n1],MPM["epsp"][:,2*n1],'g',lw=2.,ms=4.,label='usl 1ppc')
    ax2.plot(DGMPM["pos"][:,n1],DGMPM["sig"][:,n1],'r',lw=2.,ms=4.,label='dgmpm 1ppc')
    ax2.plot(MPM2["pos"][:,2*n1],MPM2["epsp"][:,2*n1],'g--',lw=2.,ms=4.,label='usl 2ppc')
    ax2.plot(DGMPM2["pos"][:,2*n1],DGMPM2["sig"][:,2*n1],'b',lw=2.,ms=4.,label='dgmpm 2ppc')
    ax2.plot(DGMPM3["pos"][:,n1],DGMPM3["sig"][:,n1],'y',lw=2.,ms=4.,label='dgmpm 2ppc (RK2)')
    ...
    export2DTeXFile(str(path)+'/EP_dgmpm_ppc_stress'+str(n1)+'.tex',np.array([MPM["pos"][:,2*n1],MPM2["pos"][:,2*n1],DGMPM["pos"][:,n1],DGMPM2["pos"][:,2*n1],DGMPM3["pos"][:,n1],DGMPM["pos"][:,n1]]),'$x (m)$',r'$\sigma (Pa)$',str(subtitle),np.array([MPM["sig"][:,2*n1],MPM2["sig"][:,2*n1],DGMPM["sig"][:,n1],DGMPM2["sig"][:,2*n1],DGMPM3["sig"][:,n1],-np.sign(v0)*Sexact]),legend)
    export2DTeXFile(str(path)+'/EP_dgmpm_ppc_epsp'+str(n1)+'.tex',np.array([MPM["pos"][:,2*n1],MPM2["pos"][:,2*n1],DGMPM["pos"][:,n1],DGMPM2["pos"][:,2*n1],DGMPM3["pos"][:,n1],DGMPM["pos"][:,n1]]),'$x (m)$',r'$\eps^p (Pa)$',str(subtitle),np.array([MPM["epsp"][:,2*n1],MPM2["epsp"][:,2*n1],DGMPM["epsp"][:,n1],DGMPM2["epsp"][:,2*n1],DGMPM3["epsp"][:,n1],-np.sign(v0)*Epexact]),legend)
...
ax1.set_ylim(1.1*np.min(DGMPM["sig"]),1.1*np.max(DGMPM["sig"]))
ax2.set_ylim(1.1*np.min(DGMPM["epsp"]),1.1*np.max(DGMPM["epsp"]))
ax1.legend(numpoints=1)
def init():
    line[0].set_data([], [])
    line[1].set_data([], [])
...
def animate(i):
    ...
    line[3].set_data(DGMPM3["pos"][:,i],DGMPM3["sig"][:,i])
    
    line[4].set_data(MPM["pos"][:,2*i],MPM["epsp"][:,2*i])
    line[5].set_data(DGMPM["pos"][:,i],DGMPM["epsp"][:,i])
    line[6].set_data(DGMPM2["pos"][:,2*i],DGMPM2["epsp"][:,2*i])

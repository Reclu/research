...
for i,n1 in enumerate(frames):
    ...
    ax1.plot(USL2["pos"][:,2*n1],USL2["sig"][:,2*n1],'b',lw=2.,ms=4.,label='USL 2ppc')
    ax1.plot(USL3["pos"][:,2*n1],USL3["sig"][:,2*n1],'g',lw=2.,ms=4.,label='USL 8ppc')
    ax1.plot(DGMPM["pos"][:,n1],DGMPM["sig"][:,n1],'r',lw=2.,ms=4.,label='DGMPM (ep solver)')
    ax1.plot(DGMPM2["pos"][:,2*n1],DGMPM2["sig"][:,2*n1],'r--',lw=2.,ms=4.,label='DGMPM (ep solver 2ppc)')
    ax1.plot(DGMPM3["pos"][:,n1],DGMPM3["sig"][:,n1],'m--',lw=2.,ms=4.,label='DGMPM (ep solver 2ppc + RK2)')
    ...
    ax2.plot(USL2["pos"][:,2*n1],USL2["epsp"][:,2*n1],'b',lw=2.,ms=4.,label='USL 2ppc')
    ax2.plot(USL3["pos"][:,2*n1],USL3["epsp"][:,2*n1],'g',lw=2.,ms=4.,label='USL 8ppc')
    ax2.plot(DGMPM["pos"][:,n1],DGMPM["epsp"][:,n1],'r',lw=2.,ms=4.,label='DGMPM (ep solver)')
    ax2.plot(DGMPM2["pos"][:,2*n1],DGMPM2["epsp"][:,2*n1],'r--',lw=2.,ms=4.,label='DGMPM (ep solver 2ppc)')
    ax2.plot(DGMPM3["pos"][:,n1],DGMPM3["epsp"][:,n1],'m--',lw=2.,ms=4.,label='DGMPM (ep solver 2ppc + RK2)')
    ...
    export2DTeXFile(str(path)+'/EP_dgmpm_ppc_stress'+str(n1)+'.tex',np.array([USL["pos"][:,2*n1],USL2["pos"][:,2*n1],DGMPM["pos"][:,n1],DGMPM2["pos"][:,2*n1],DGMPM3["pos"][:,n1],DGMPM["pos"][:,n1]]),'$x (m)$',r'$\sigma (Pa)$',str(subtitle),np.array([USL["sig"][:,2*n1],USL2["sig"][:,2*n1],DGMPM["sig"][:,n1],DGMPM2["sig"][:,2*n1],DGMPM3["sig"][:,n1],-np.sign(v0)*Sexact]),legend)
    export2DTeXFile(str(path)+'/EP_dgmpm_ppc_epsp'+str(n1)+'.tex',np.array([USL["pos"][:,2*n1],USL2["pos"][:,2*n1],DGMPM["pos"][:,n1],DGMPM2["pos"][:,2*n1],DGMPM3["pos"][:,n1],DGMPM["pos"][:,n1]]),'$x (m)$',r'$\eps^p$',str(subtitle),np.array([USL["epsp"][:,2*n1],USL2["epsp"][:,2*n1],DGMPM["epsp"][:,n1],DGMPM2["epsp"][:,2*n1],DGMPM3["epsp"][:,n1],-np.sign(v0)*Epexact]),legend)
...
ax1.set_ylim(1.1*np.min(DGMPM2["sig"]),1.1*np.max(DGMPM2["sig"]))
ax2.set_ylim(1.1*np.min(DGMPM2["epsp"]),1.1*np.max(DGMPM2["epsp"]))
ax1.legend(numpoints=1)
def init():
    line[0].set_data([], [])
    line[1].set_data([], [])

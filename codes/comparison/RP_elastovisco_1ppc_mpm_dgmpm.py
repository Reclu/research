...
line3, = ax1.plot([], [],'k' ,ms=1.5,label='fvm')
line4, = ax2.plot([], [],'r-o', ms=2.5,label='dgmpm')
line5, = ax2.plot([], [],'g--' ,ms=1.5,label='mpm')
line6, = ax2.plot([], [],'k' ,ms=1.5,label='fvm')
...
if hardening=='kinematic':
    ...
    ax2.set_ylim(1.1*np.min(MPM["p"]),1.1*np.max(MPM["p"]))
...
def animate(i):
    ...
    line[2].set_data(FVM["centroids"],FVM["sig"][:,i])
    line[4].set_data(MPM["pos"][:,2*i],MPM["epsp"][:,2*i])
    line[3].set_data(DGMPM["pos"][:,i],DGMPM["epsp"][:,i])

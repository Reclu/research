...
% \review{hardening exponent value for nonlinear hardening}
\begin{table}[h!]
  \centering
...
Linear elements and a lumped mass matrix are used within FEM with no artificial viscosity added, and the regular mesh is built so that Gauss' points are consistent with the particles.
...
It is worth noticing that the stability properties of the DGMPM using 2ppc can be improved by using a second-order Runge-Kutta time integration instead of the forward Euler algorithm \cite{DGMPM}.
Then, a second-order time integration would lead to the computation of sharper and hence more accurate solutions with the DGMPM(2ppc).
